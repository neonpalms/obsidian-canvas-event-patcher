...
export default class CanvasEventPatcher {
  ...
  private static tryApplyCanvasPatch() {
    ...
    CanvasEventPatcher.plugin.register(
      around(canvas.constructor.prototype, {
        ...
        updateSelection: (next: any) =>
          function (update: () => void) {
            ...
            workspace.trigger(
              ...
              oldSelection,
              (update: () => void) => next.call(this, update),
            );
        ...
        createGroupNode: (next: any) =>
          function (group: CanvasGroupData) {
        ...
        removeNode: (next: any) =>
          function (node: AllCanvasNodeData) {
            if (node.zIndex < -999) {
        ...
        addEdge: (next: any) =>
          function (edge: CanvasEdgeData) {
            workspace.trigger(CanvasEvent.EdgeCreated, edge);
            const result = next.call(this, edge);
            console.info(
              `Edge ${edge.id} created between nodes ${edge.fromNode} to ${edge.toNode}.`,
            );
            return result;
          },
        removeEdge: (next: any) =>
          function (edge: CanvasEdgeData) {
            console.info(
              `Deleted edge ${edge.id} between nodes ${edge.fromNode} to ${edge.toNode}.`,
            );
            workspace.trigger(CanvasEvent.EdgeRemoved, edge);
            const result = next.call(this, edge);
            return result;
          },
        setReadonly: (next: any) =>
          function (readonly: boolean) {
            readonly
              ? console.info(`Readonly mode on.`)
              : console.info(`Readonly mode off.`);
    ...
    CanvasEventPatcher.plugin.register(
      around(canvas.nodeInteractionLayer.constructor.prototype, {
        setTarget: (next: any) =>
          function (node: CanvasNodeData) {
            const result = next.call(this, node);
